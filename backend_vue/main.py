...
POSTGRES_MIGRATIONS_DIR = BASE_DIR / "migrations" / "ai_lounge"
...
@app.get("/api/assets/{asset_id}/registration-template")
def get_ai_asset_registration_template(
    asset_id: str,
    current_user: Annotated[UserResponse, Depends(get_current_user)],
) -> dict[str, Any]:
    asset_id = validate_asset_id(asset_id)
    with get_connection() as con:
        row = con.execute(
            """
            SELECT a.*, u.displayed_name AS owner_name, u.org_name AS owner_org,
                   u.job_title AS owner_job_title, FALSE AS is_bookmarked
            FROM ai_assets a
            JOIN portal_users u ON u.user_id = a.created_by
            WHERE a.asset_id = ?
              AND a.approval_status = ? AND a.is_active = 1
            """,
            (asset_id, "approved"),
        ).fetchone()
        if row is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="템플릿으로 사용할 AI 자산을 찾을 수 없습니다.")
