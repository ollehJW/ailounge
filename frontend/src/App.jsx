import React, { lazy, Suspense, useEffect, useEffectEvent, useMemo, useRef, useState } from 'react';
import LoginPage from './components/LoginPage';
import ManagedNewsPreviewModal from './components/ManagedNewsPreviewModal';
import { API_BASE, AUTH_EXPIRED_EVENT, apiError, apiFetch } from './api/client';
...
const AiUsageEditor = lazy(() => import('./components/AiUsageEditor'));
...
const releaseAssetImagePreviews = (items) => {
  items.forEach((item) => {
...
const dxMessagesFromApi = (messages = []) => {
  const history = messages.map((message) => ({ role: message.role, text: message.content }));
  const greeting = dxInitialMessages[0];
  const alreadyIncludesGreeting = history[0]?.role === greeting.role && history[0]?.text === greeting.text;
  return alreadyIncludesGreeting ? history : [...dxInitialMessages, ...history];
};
...
function App() {
  ...
  const assetDetailAbortRef = useRef(null);
  const newsDetailAbortRef = useRef(null);
  const usageDetailAbortRef = useRef(null);
  const dxSessionAbortRef = useRef(null);
  const dxChatAbortRef = useRef(null);
  ...
  useEffect(() => {
    window.addEventListener(AUTH_EXPIRED_EVENT, clearSessionAndReload);
    return () => window.removeEventListener(AUTH_EXPIRED_EVENT, clearSessionAndReload);
  }, []);

  useEffect(() => () => {
    assetDetailAbortRef.current?.abort();
    newsDetailAbortRef.current?.abort();
    usageDetailAbortRef.current?.abort();
    dxSessionAbortRef.current?.abort();
    dxChatAbortRef.current?.abort();
  }, []);
  ...
  const loadAccountsOnEntry = useEffectEvent(() => loadAccounts());
  const loadNewsListOnEntry = useEffectEvent(() => loadNewsList());
  const loadNewsDetailOnSelect = useEffectEvent((newsId) => loadNewsDetail(newsId));
  const loadAiUsagePostsOnEntry = useEffectEvent(() => loadAiUsagePosts());
  const loadAiIdeasOnEntry = useEffectEvent(() => loadAiIdeas());
  const loadAdminIdeasOnEntry = useEffectEvent(() => loadAdminIdeas());
  const loadAdminAssetsOnEntry = useEffectEvent(() => loadAdminAssets());
  const loadIntroSummaryOnEntry = useEffectEvent(() => loadIntroSummary());
  const loadAssetCatalogOnEntry = useEffectEvent(() => loadAssetCatalog());
  const loadDxSessionsOnEntry = useEffectEvent(() => loadDxSessions());
  const loadAiUsageDetailOnSelect = useEffectEvent((postId) => loadAiUsagePostDetail(postId));
  const loadMyAiAssetsOnEntry = useEffectEvent(() => loadMyAiAssets());
  const refreshAssetSpecOnChange = useEffectEvent(() => refreshAssetSpecReady());
  ...
  useEffect(() => {
    if (isAdminView) loadAccountsOnEntry();
  }, [isAdminView]);

  useEffect(() => {
    if (authUser && ((!isAdminView && activePage === 'tech-news') || (isAdminView && activePage === 'tech-news-write'))) loadNewsListOnEntry();
  }, [authUser, isAdminView, activePage]);

  useEffect(() => {
    if (selectedNewsId) loadNewsDetailOnSelect(selectedNewsId);
    else {
      newsDetailAbortRef.current?.abort();
      newsDetailAbortRef.current = null;
  ...
  useEffect(() => {
    if (authUser && !isAdminView && activePage === 'ai-blog') loadAiUsagePostsOnEntry();
  }, [authUser, isAdminView, activePage]);

  useEffect(() => {
    if (authUser && !isAdminView && activePage === 'gen-ai-proposal') loadAiIdeasOnEntry();
  }, [authUser, isAdminView, activePage]);

  useEffect(() => {
    if (authUser && isAdminView && activePage === 'idea-review') loadAdminIdeasOnEntry();
  }, [authUser, isAdminView, activePage]);

  useEffect(() => {
    if (authUser && isAdminView && activePage === "asset-management") loadAdminAssetsOnEntry();
  }, [authUser, isAdminView, activePage]);

  useEffect(() => {
    if (authUser && !isAdminView && activePage === 'intro') loadIntroSummaryOnEntry();
  }, [authUser, isAdminView, activePage]);

  useEffect(() => {
    if (authUser && !isAdminView && activePage === 'explore') loadAssetCatalogOnEntry();
  }, [authUser, isAdminView, activePage]);

  useEffect(() => {
    if (authUser && !isAdminView && activePage === 'dx-discovery') loadDxSessionsOnEntry();
  }, [authUser, isAdminView, activePage]);

  useEffect(() => {
    if (selectedAiUsagePostId) loadAiUsageDetailOnSelect(selectedAiUsagePostId);
    else {
      usageDetailAbortRef.current?.abort();
      usageDetailAbortRef.current = null;
  ...
  const startSkillGeneration = async () => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}/api/assets/${assetId}/skill-plan`, {
        method: 'POST',
        headers: authHeaders,
      });
  ...
  const confirmGeneratedSkillSelection = async () => {
    ...
    try {
      const response = await apiFetch(API_BASE + "/api/assets/" + assetId + "/skills/generate", {
        method: "POST",
  ...
  const resetAssetRegistry = () => {
    releaseAssetImagePreviews(assetImageItems);
  ...
  const loadIntroSummary = async () => {
    ...
    try {
      const response = await apiFetch(API_BASE + '/api/assets/intro/summary', { headers: authHeaders });
  ...
  const loadAssetCatalog = async () => {
    ...
    try {
      const response = await apiFetch(API_BASE + '/api/assets/catalog', { headers: authHeaders });
  ...
  const requestAssetRecommendations = async (event) => {
    ...
    try {
      const response = await apiFetch(API_BASE + '/api/assets/recommendations', {
        method: 'POST',
        headers: { ...authHeaders, 'Content-Type': 'application/json' },
  ...
  const toggleAssetBookmark = async (assetId, event) => {
    ...
    try {
      const response = await apiFetch(API_BASE + '/api/assets/catalog/' + assetId + '/bookmark', {
  ...
  const openAssetCatalogDetail = async (asset) => {
    assetDetailAbortRef.current?.abort();
    const controller = new AbortController();
    assetDetailAbortRef.current = controller;
    ...
    try {
      const response = await apiFetch(API_BASE + '/api/assets/catalog/' + asset.asset_id, {
        headers: authHeaders,
        signal: controller.signal,
      });
      ...
      if (error.name !== 'AbortError') setCatalogDetailError(error.message);
    } finally {
      if (assetDetailAbortRef.current === controller) {
        assetDetailAbortRef.current = null;
  ...
  const loadAssetDiffusionCases = async (assetId) => {
    ...
    setDiffusionCaseError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/assets/catalog/${assetId}/diffusion-cases`, { headers: authHeaders });
  ...
  const loadAssetQa = async (assetId) => {
    ...
    setAssetQaError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/assets/catalog/${assetId}/qa`, { headers: authHeaders });
  ...
  const submitAssetQaQuestion = async (event) => {
    ...
    setAssetQaError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/assets/catalog/${assetId}/qa/questions`, {
        method: 'POST',
        headers: { ...authHeaders, 'Content-Type': 'application/json' },
  ...
  const saveAssetQaQuestion = async (event, questionId) => {
    ...
    setAssetQaError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/assets/catalog/${assetId}/qa/posts/${questionId}`, {
        method: 'PUT',
        headers: { ...authHeaders, 'Content-Type': 'application/json' },
        body: JSON.stringify({ content }),
      });
  ...
  const deleteAssetQaQuestion = async (question) => {
    ...
    setAssetQaError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/assets/catalog/${assetId}/qa/posts/${question.qa_post_id}`, {
        method: 'DELETE',
        headers: authHeaders,
      });
  ...
  const openAssetRegistrationDocument = async (asset) => {
    ...
    try {
      const response = await apiFetch(API_BASE + "/api/assets/" + asset.asset_id + "/registration-document", { headers: authHeaders });
  ...
  const loadMyAiAssets = async () => {
    ...
    setMyAiAssetsError("");
    try {
      const response = await apiFetch(API_BASE + "/api/assets/mine", { headers: authHeaders });
  ...
  const loadAdminAssets = async () => {
    ...
    setAdminAssetsError("");
    try {
      const response = await apiFetch(API_BASE + "/api/admin/assets", { headers: authHeaders });
  ...
  const submitAssetReview = async (event) => {
    ...
    setAdminAssetsError("");
    try {
      const response = await apiFetch(API_BASE + "/api/admin/assets/" + assetReviewTarget.asset_id + "/status", {
        method: "PUT",
        headers: { ...authHeaders, "Content-Type": "application/json" },
  ...
  const toggleAdminAssetActivation = async (asset) => {
    ...
    setAdminAssetsError("");
    try {
      const response = await apiFetch(API_BASE + "/api/admin/assets/" + asset.asset_id + "/activation", {
        method: "PUT",
        headers: { ...authHeaders, "Content-Type": "application/json" },
  ...
  const deleteAdminAsset = async () => {
    ...
    setAdminAssetsError("");
    try {
      const response = await apiFetch(API_BASE + "/api/admin/assets/" + assetDeleteTarget.asset_id, {
        method: "DELETE",
        headers: authHeaders,
      });
  ...
  useEffect(() => {
    if (authUser && !isAdminView && activePage === 'registry') loadMyAiAssetsOnEntry();
  }, [authUser, isAdminView, activePage]);
  useEffect(() => {
    if (activePage !== 'registry' || assetRegistryStep !== 0) return undefined;
    const timer = window.setTimeout(refreshAssetSpecOnChange, 0);
  ...
  const stageAssetSpecification = async () => {
    ...
    try {
      ...
      const response = await apiFetch(`${API_BASE}/api/assets/staging`, {
        method: 'POST',
        headers: authHeaders,
  ...
  const cloneAssetRepository = async () => {
    ...
    setAssetRepoErrorMessage('');
    try {
      const response = await apiFetch(`${API_BASE}/api/assets/repository/clone`, {
        method: 'POST',
        headers: { ...authHeaders, 'Content-Type': 'application/json' },
  ...
  const submitAssetRegistration = async () => {
    ...
    try {
      ...
      const payload = attachAssetSlidesMeta(buildAssetPayload(livePayload), livePayload);
      validateAssetUploadSizes();
      const response = await apiFetch(`${API_BASE}/api/assets`, {
        method: 'POST',
        headers: authHeaders,
        body: buildAssetFormData(payload),
      });
  ...
  const applyDxSessionDetail = (detail) => {
    ...
    setDxMessages(dxMessagesFromApi(detail.messages));
  ...
  const loadDxSessions = async () => {
    ...
    setDxError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/dx-discovery/sessions`, { headers: authHeaders });
  ...
  const createDxSession = async (refreshList = true) => {
    ...
    setDxInput('');
    try {
      const response = await apiFetch(`${API_BASE}/api/dx-discovery/sessions`, {
        method: 'POST',
        headers: authHeaders,
      });
  ...
  const openDxSession = async (sessionId, showLoading = true) => {
    ...
    dxChatAbortRef.current?.abort();
    const controller = new AbortController();
    dxSessionAbortRef.current = controller;
    if (showLoading) setIsLoadingDxSessions(true);
    setDxError('');
    try {
      const response = await apiFetch(API_BASE + '/api/dx-discovery/sessions/' + sessionId, {
        headers: authHeaders,
        signal: controller.signal,
      });
      if (!response.ok) throw await apiError(response, 'DX 과제 발굴 세션을 불러오지 못했습니다.');
      const detail = await response.json();
      if (!controller.signal.aborted) applyDxSessionDetail(detail);
    } catch (error) {
      if (error.name !== 'AbortError') setDxError(error.message);
    } finally {
      if (dxSessionAbortRef.current === controller) {
        dxSessionAbortRef.current = null;
  ...
  const submitDxMessage = async (event) => {
    ...
    dxChatAbortRef.current?.abort();
    const controller = new AbortController();
    dxChatAbortRef.current = controller;

    try {
      const response = await apiFetch(`${API_BASE}/api/dx-discovery/sessions/${sessionId}/chat`, {
        method: 'POST',
        headers: { ...authHeaders, 'Content-Type': 'application/json' },
      ...
      if (dxChatAbortRef.current === controller) {
        dxChatAbortRef.current = null;
        setIsDxTyping(false);
      }
    }
  };
  ...
  const loadAccounts = async () => {
    ...
    setAccountError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/admin/users`, { headers: authHeaders });
  ...
  const loadNewsList = async () => {
    ...
    setNewsError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/news`);
  ...
  const fetchNewsDetail = async (newsId, { countView = true, signal } = {}) => {
    const response = await apiFetch(API_BASE + '/api/news/' + newsId + '?count_view=' + (countView ? 'true' : 'false'), { signal });
  ...
  const loadNewsDetail = async (newsId) => {
    newsDetailAbortRef.current?.abort();
    const controller = new AbortController();
    newsDetailAbortRef.current = controller;
    setSelectedNews(null);
    setNewsError('');
    try {
      const detail = await fetchNewsDetail(newsId, { signal: controller.signal });
      ...
      if (newsDetailAbortRef.current === controller) newsDetailAbortRef.current = null;
    }
  };
  ...
  const loadAiUsagePosts = async () => {
    ...
    setAiUsageError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/usage-posts`, { headers: authHeaders });
  ...
  const loadAiUsagePostDetail = async (usagePostId, { countView = true, openDetail = true } = {}) => {
    usageDetailAbortRef.current?.abort();
    const controller = new AbortController();
    usageDetailAbortRef.current = controller;
    if (openDetail) setSelectedAiUsagePost(null);
    setAiUsageError('');
    try {
      const url = API_BASE + '/api/usage-posts/' + usagePostId + '?count_view=' + (countView ? 'true' : 'false');
      const response = await apiFetch(url, { headers: authHeaders, signal: controller.signal });
      ...
      if (usageDetailAbortRef.current === controller) usageDetailAbortRef.current = null;
    }
  };
  ...
  const publishNews = async (event) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}/api/admin/news${editingNewsId ? `/${editingNewsId}` : ''}`, {
  ...
  const draftNewsMarkdown = async () => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}/api/admin/news/draft`, {
        method: 'POST',
        headers: { ...authHeaders, 'Content-Type': 'application/json' },
  ...
  const deleteNews = async (news) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}/api/admin/news/${news.news_id}`, {
        method: 'DELETE',
        headers: authHeaders,
      });
  ...
  const loadAiIdeas = async () => {
    ...
    setAiIdeaError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/ideas`, { headers: authHeaders });
  ...
  const loadAdminIdeas = async () => {
    ...
    setAdminIdeaError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/admin/ideas`, { headers: authHeaders });
  ...
  const submitIdeaReview = async (event) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}/api/admin/ideas/${ideaReviewTarget.idea_id}/status`, {
        method: 'PUT',
        headers: { ...authHeaders, 'Content-Type': 'application/json' },
  ...
  const downloadIdeaAttachment = async (attachment) => {
    ...
    setAiIdeaError('');
    try {
      const response = await apiFetch(`${API_BASE}${attachment.url}`, { headers: authHeaders });
  ...
  const deleteAiIdea = async () => {
    ...
    setAiIdeaError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/ideas/${ideaToDelete.idea_id}`, {
        method: 'DELETE',
        headers: authHeaders,
      });
  ...
  const submitAiIdea = async (event) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}/api/ideas`, {
        method: 'POST',
        headers: authHeaders,
        body: formData,
      });
  ...
  const submitAiUsagePost = async (event) => {
    event.preventDefault();
    const content = aiUsageForm.content;
    ...
    setAiUsageError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/usage-posts${editingAiUsagePostId ? `/${editingAiUsagePostId}` : ''}`, {
  ...
  const toggleAiUsageLike = async (postId, event) => {
    ...
    setAiUsageError('');
    try {
      const response = await apiFetch(`${API_BASE}/api/usage-posts/${postId}/like`, {
        method: 'POST',
        headers: authHeaders,
      });
  ...
  const saveAccount = async (event) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}/api/admin/users${editingUserId ? `/${editingUserId}` : ''}`, {
  ...
  const deleteAccount = async (account) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}/api/admin/users/${account.user_id}`, {
        method: 'DELETE',
        headers: authHeaders,
      });
              ...
              <div className="news-modal-backdrop" role="presentation" onMouseDown={closeAiUsageComposer}>
                <form className="ai-usage-compose-modal" onSubmit={submitAiUsagePost} onMouseDown={(event) => event.stopPropagation()}>
                  ...
                  <div className="form-field">
                    <span>내용</span>
                    <Suspense fallback={<div className="ai-usage-rich-editor loading" />}>
                      <AiUsageEditor
                        value={aiUsageForm.content}
                        onChange={(content) => updateAiUsageField('content', content)}
                      />
                    </Suspense>
                  </div>
