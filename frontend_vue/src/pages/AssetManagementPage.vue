...
async function submitReview(){saving.value=true;try{const r=await apiFetch(`/api/admin/assets/${reviewTarget.value.asset_id}/status`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({status:reviewForm.status,review_comment:reviewForm.comment.trim()})});if(!r.ok)throw await readApiError(r,"자산 심사를 완료하지 못했습니다.");const u=await r.json();assets.value=assets.value.map(a=>a.asset_id===u.asset_id?u:a);reviewTarget.value=null;reviewForm.status="";reviewForm.comment="";}catch(e){error.value=e.message;}finally{saving.value=false;}}
async function toggleActivation(a){busyId.value=a.asset_id;error.value="";try{const r=await apiFetch(`/api/admin/assets/${a.asset_id}/activation`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({is_active:!a.is_active})});if(!r.ok)throw await readApiError(r,"활성 상태를 변경하지 못했습니다.");const u=await r.json();assets.value=assets.value.map(x=>x.asset_id===u.asset_id?u:x);}catch(e){error.value=e.message;}finally{busyId.value="";}}
async function removeAsset(){saving.value=true;error.value="";try{const r=await apiFetch(`/api/admin/assets/${deleteTarget.value.asset_id}`,{method:"DELETE"});if(!r.ok)throw await readApiError(r,"자산을 삭제하지 못했습니다.");assets.value=assets.value.filter(a=>a.asset_id!==deleteTarget.value.asset_id);deleteTarget.value=null;}catch(e){error.value=e.message;}finally{saving.value=false;}}
onMounted(load);
</script>
