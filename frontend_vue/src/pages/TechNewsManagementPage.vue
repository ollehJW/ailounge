...
async function createDraft(){drafting.value=true;error.value="";try{const r=await apiFetch("/api/admin/news/draft",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({source:form.source.trim(),category:form.category,title:form.title.trim()||null,org_name:form.org_name.trim()||null})});if(!r.ok)throw await readApiError(r,"마크다운을 자동 작성하지 못했습니다.");form.markdown=(await r.json()).markdown;draftReady.value=true;}catch(e){error.value=e.message;}finally{drafting.value=false;}}
...
async function publish(){publishing.value=true;error.value="";try{const r=await apiFetch(editingId.value?`/api/admin/news/${editingId.value}`:"/api/admin/news",{method:editingId.value?"PUT":"POST",body:buildForm()});if(!r.ok)throw await readApiError(r,editingId.value?"뉴스를 수정하지 못했습니다.":"뉴스를 발행하지 못했습니다.");reset();await load();tab.value="manage";}catch(e){error.value=e.message;}finally{publishing.value=false;}}
