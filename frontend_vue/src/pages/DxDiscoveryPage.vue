      ...
      <div class="dx-workspace">
        <aside class="dx-sessions">
          ...
          <div class="dx-session-list">
            ...
            <button v-for="session in sessions" :key="session.session_id" :class="{active:session.session_id===activeId}" @click="openSession(session.session_id)"><strong>{{ session.title }}</strong><small><Clock3 :size="11" />{{ date(session.updated_at) }}<em :class="session.status==='과제 발굴 완료'?'done':'progress'">{{ session.status }}</em></small><span title="이력 삭제" @click.stop="removeSession(session.session_id)"><Trash2 :size="14" /></span></button>
            <div v-if="!loading&&!sessions.length" class="dx-session-empty">저장된 채팅 이력이 없습니다.</div>
          </div>
        </aside>

        <section class="dx-chat-panel">
          <header><span></span><div><strong>DX 과제 발굴 Agent</strong><small>업무를 설명하면 필요한 정보를 차례로 확인합니다.</small></div></header>
          ...
          <form @submit.prevent="sendMessage"><textarea v-model="input" rows="1" :disabled="typing" placeholder="해결하고 싶은 업무 과제를 자유롭게 설명해 주세요." @keydown.enter.exact.prevent="sendMessage"></textarea><button class="primary-button" :disabled="typing||!input.trim()" aria-label="전송"><Send :size="17" /></button></form>
        </section>
      </div>
...
const sessions=ref([]),detail=ref(null),activeId=ref(""),input=ref(""),loading=ref(true),typing=ref(false),error=ref(""),chatBody=ref(null);
...
async function createSession(){error.value="";try{const created=await request("/api/dx-discovery/sessions",{method:"POST"},"새 채팅을 만들지 못했습니다.");sessions.value=[created,...sessions.value];detail.value=created;activeId.value=created.session_id;await scrollBottom();}catch(e){error.value=e.message;}}
...
async function removeSession(id){try{await request(`/api/dx-discovery/sessions/${id}`,{method:"DELETE"},"이력을 삭제하지 못했습니다.");sessions.value=sessions.value.filter(item=>item.session_id!==id);if(activeId.value===id){detail.value=null;activeId.value="";if(sessions.value.length)await openSession(sessions.value[0].session_id);else await createSession();}}catch(e){error.value=e.message;}}
async function sendMessage(){const text=input.value.trim();if(!text||typing.value)return;if(!activeId.value)await createSession();input.value="";typing.value=true;error.value="";const optimistic={message_id:`local-${Date.now()}`,role:"user",content:text};detail.value={...detail.value,messages:[...(detail.value?.messages||[]),optimistic]};await scrollBottom();try{const updated=await request(`/api/dx-discovery/sessions/${activeId.value}/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:[{role:"user",text}]})},"Agent 응답을 불러오지 못했습니다.");detail.value=updated;sessions.value=[updated,...sessions.value.filter(item=>item.session_id!==updated.session_id)];}catch(e){error.value=e.message;}finally{typing.value=false;await scrollBottom();}}
