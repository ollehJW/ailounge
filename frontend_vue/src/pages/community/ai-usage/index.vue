...
const renderedPostContent = computed(() => resolveApiHtml(selectedPost.value?.content_html));
...
const submitPost=async()=>{syncContent();if(!form.title.trim()||!form.content_html.replace(/<[^>]*>/g,"").trim()){composerError.value="제목과 내용을 모두 작성해 주세요.";return;}saving.value=true;composerError.value="";try{const response=await apiFetch(`/api/usage-posts${editingId.value?`/${editingId.value}`:""}`,{method:editingId.value?"PUT":"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${auth.token}`},body:JSON.stringify({title:form.title.trim(),category:form.category,content_html:normalizeApiHtml(form.content_html)})});if(!response.ok)throw await readApiError(response,editingId.value?"게시글을 수정하지 못했습니다.":"게시글을 등록하지 못했습니다.");await loadPosts();closeComposer();}catch(saveError){composerError.value=saveError.message;}finally{saving.value=false;}};
