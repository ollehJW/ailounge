...
<script setup>
import { computed,onMounted,ref,watch } from "vue";import { useRoute,useRouter } from "vue-router";import { ArrowRight,LoaderCircle,Search,Sparkles,Star,Wand2 } from "@/icons/lucide";import AssetCard from "@/components/AssetCard.vue";import AssetDetailDrawer from "@/components/AssetDetailDrawer.vue";import { apiFetch,readApiError } from "@/api/client";import { useAuthStore } from "@/stores/auth";
...
async function toggleBookmark(asset){const next=!asset.is_bookmarked;try{await call(`/api/assets/catalog/${asset.asset_id}/bookmark`,{method:next?"POST":"DELETE",headers:{Authorization:`Bearer ${auth.token}`}},"컬렉션을 변경하지 못했습니다.");asset.is_bookmarked=next;}catch(e){error.value=e.message;}}
async function recommend(){recommending.value=true;recommendationAttempted.value=false;recommendError.value="";recommendations.value=[];try{const result=await call("/api/assets/recommendations",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${auth.token}`},body:JSON.stringify({query:recommendQuery.value.trim()})},"자산을 추천하지 못했습니다.");recommendations.value=result.recommendations||[];}catch(e){recommendError.value=e.message;}finally{recommendationAttempted.value=true;recommending.value=false;}}
