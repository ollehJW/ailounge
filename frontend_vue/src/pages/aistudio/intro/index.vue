...
const entries=[{to:"/aistudio/dx-discovery",number:"01",eyebrow:"DEFINE",title:"업무 과제 구체화",description:"Agent와 대화하며 현장의 문제를 실행 가능한 DX 과제로 구체화합니다.",action:"DX 과제 발굴로 이동",icon:Sparkles},{to:"/aistudio/assets/explore",number:"02",eyebrow:"REUSE",title:"검증 자산 탐색·확산",description:"승인된 운영 자산의 코드, Skill과 실제 적용 경험을 확인합니다.",action:"AI 자산 탐색으로 이동",icon:Search},{to:"/aistudio/assets/register",number:"03",eyebrow:"SHARE",title:"완성 자산 등록·공유",description:"코드·데이터·활용 화면을 재사용 가능한 자산으로 등록합니다.",action:"AI 자산 등록으로 이동",icon:Plus}];
...
const openAsset=assetId=>router.push({path:"/aistudio/assets/explore",query:{asset:assetId}});onMounted(loadSummary);
</script>
