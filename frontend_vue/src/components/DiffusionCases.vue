...
async function save(){if(!form.title||!form.stage||!form.applied_work||!form.customization||!form.effect){error.value="필수 항목을 모두 작성하세요.";return;}saving.value=true;try{const path=`/api/assets/catalog/${props.asset.asset_id}/diffusion-cases${editingId.value?`/${editingId.value}`:""}`;const result=await call(path,{method:editingId.value?"PUT":"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${auth.token}`},body:JSON.stringify(form)},"사례를 저장하지 못했습니다.");emit("count-change",result.diffusion_completed_count);formOpen.value=false;await load();}catch(e){error.value=e.message;}finally{saving.value=false;}}
async function remove(item){if(!confirm("이 확산 사례를 삭제할까요?"))return;try{const result=await call(`/api/assets/catalog/${props.asset.asset_id}/diffusion-cases/${item.diffusion_case_id}`,{method:"DELETE",headers:{Authorization:`Bearer ${auth.token}`}},"사례를 삭제하지 못했습니다.");emit("count-change",result.diffusion_completed_count);await load();}catch(e){error.value=e.message;}}onMounted(load);
</script>
